...
| # | Tarea | Nota |
|---|---|---|
| 39 | **Que el CLIENTE pueda configurar su propio agente** | 🟡 **La mitad quedó lista el 28-ago:** los campos ya los maneja el cliente (fila 52). **Falta el tono** — cómo habla su agente sigue viviendo en la consola de ToqueFlow. Es lo otro que va a querer ajustar cada semana | Diego |
...
| 50 | **Panel de consumo por cliente** | Tokens, costo en USD y por mes. **Los datos ya existen:** la tabla `ai_usage` registra `input_tokens`, `output_tokens`, `cost_usd` y `model` por empresa desde hace rato. Falta solo la pantalla |
...
| 85 | **Borrar las 5 copias de `_test_exceljs_tmp`** | Basura de una prueba de mayo. Esas sí se borran |
...
| 102 | 🟠 **Cinco webhooks de la plataforma no comprueban quién llama** | `toque-campana-test`, `toque-pago-test`, `toque-msgprueba-test` y `alerta-recursos`. Los tres primeros son los manejadores del **sandbox**: quien acierte la URL puede meterle mensajes falsos al simulador de un cliente, o disparar el manejador de campañas de prueba. El de alertas deja mandar avisos falsos del VPS. Ninguno toca producción, pero la URL es adivinable y el arreglo es el mismo que ya se hizo en el agente: exigir `X-Toque-Signature`. **Son míos, los arreglo yo** | Claude |
| 103 | 🟡 **45 webhooks abiertos en flujos viejos de clientes** | Anteriores al estándar: FerreteríaYa, Savia, LuxeSmile, Zoe, Bejauha y los de contenido. Hay que mirarlos **uno por uno**, no en bloque: los de WhatsApp dejan inyectar mensajes falsos —el mismo bug que se arregló en el agente nuevo, que es cómo un tercero hace que el WhatsApp de un cliente escriba a quien él elija— pero los de formularios y páginas puede que estén bien abiertos. Se cruza con normalizar a esos clientes (fila 26) | Diego |
| 104 | 🟡 **61 de 68 flujos activos no avisan si fallan** | Sin `Error Trigger` ni `errorWorkflow`. Fallan en silencio, y eso no es teoría: **así lleva días caído el WhatsApp de FerreteríaYa sin que nadie se enterara** (fila 57). Se cruza con la fila 66 | Diego |
| 105 | 🟡 **El registro de Supabase está abierto** | Cualquiera con la llave pública puede crearse una cuenta sin que nadie lo invite. Comprobado con `auditoria-registro-abierto.cjs`: **un colado no alcanza nada** —cero filas en empresas, contactos, conversaciones, campañas, agentes, conocimiento, consumo y perfiles— así que no es una puerta. Pero cada cuenta consume cupo y ensucia la lista de usuarios. Se cierra en Supabase → Authentication → Sign In / Providers | Diego |
| 106 | `pago-webhook` y `cargar-conocimiento` **no están desplegadas** | Las dos devuelven 404 en producción aunque el código existe y el CLAUDE.md dice que están. Necesitan `SUPABASE_ACCESS_TOKEN` (fila 8) | Diego |
