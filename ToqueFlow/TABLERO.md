...
|---|---|---|---|
| 17 | 🟡 **Construir las herramientas que faltan** | ✅ **`ver-disponibilidad` y `agendar-cita`, hechas el 30-ago** — son las que cierran la cita dentro de la conversación, donde está el valor para una clínica o un spa. Se apoyan en la agenda de la fila 34. **Lo que decidió el diseño: no puede agendar dos personas en el mismo cupo.** Que el agente ofrezca una hora y otro la tome medio segundo antes no es raro, es lo normal en un negocio que funciona — y el resultado sería alguien que llega y no lo pueden atender, que es el peor fallo posible porque el cliente del cliente lo ve en la cara. Por eso la disponibilidad se comprueba AL ESCRIBIR, en la misma transacción y con candado por empresa. Probado lanzando **4 citas simultáneas por el mismo cupo: entra una**. Y las fechas salen en español — con `TMDay` decía «Wednesday 02 de September», porque dependía de la configuración regional del servidor. **Faltan** la familia del saldo (matricular → descontar → recargar), `estado-pedido` y `confirmar-pago` (existen para FerreteríaYa y Savia como flujos aparte: exponerlas es menos trabajo que construirlas) y `registrar-reclamo`. Matricular y recargar exigen confirmación de una persona por diseño: un agente que recarga solo es un agente que regala | Diego |
...
| 108 | ⭐ **Lo nuevo tiene pruebas, pero hay tres huecos concretos** | Todo lo construido el 29 y el 30 quedó con prueba propia —campos, tono, conocimiento, agenda, agendar, rastreador— y las 19 pasan. **Lo que NO está cubierto, dicho sin adornos:** (a) **ninguna conversación usa las herramientas nuevas.** Se probó que `agendar-cita` funciona llamándola directo, pero no que el AGENTE sepa cuándo llamarla, con qué fecha, ni qué contesta si la hora se ocupó — que es donde falla un modelo; (b) **las pantallas nuevas solo se comprueban a nivel de que compilan.** Que el cliente pueda escribir su conocimiento está probado contra la base, pero nadie ha comprobado que el botón haga lo que dice; (c) **el paso de IA del cargador nunca se ha corrido** — falta la llave de Anthropic (fila 6). Y una cuarta que es de hábito, no de cobertura: **la auditoría es una foto, no una costumbre** — se corre cuando alguien se acuerda (fila 33) | Claude |
