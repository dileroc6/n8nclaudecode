...
|---|---|---|---|
| 17 | ⭐ **Construir las herramientas que faltan** | De las 11 que puede llevar el agente, **solo 1 está liberada**. En orden de retorno: **`estado-pedido` y `confirmar-pago` ya existen** para FerreteríaYa y Savia como flujos aparte —exponerlas es menos trabajo que construirlas—; **`registrar-consumo` y `recargar-saldo`** completan la familia del saldo, que hoy está coja (consultar sin poder descontar deja el número desactualizado); y **`ver-disponibilidad` + `agendar-cita`** son las que cierran la cita dentro de la conversación, que es donde está el valor | Claude |
