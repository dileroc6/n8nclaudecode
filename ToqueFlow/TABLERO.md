...
| 143 | 🚨 **La llave de Evolution es adivinable: `leadai2024secreto`** | Apareció al abrir el compose para sacarla de ahí. **No es una llave, es una frase**: el nombre del proyecto viejo más el año. Y protege la API de Evolution, que está publicada en `evolution.srv1398596.hstgr.cloud` — quien la acierte **manda WhatsApp como cualquier cliente conectado**. Son dos problemas distintos y este es el peor: estar mal guardada ya se arregló (fila 118), **ser adivinable no**. **Ahora rotarla es barato:** todos los flujos la leen de un solo sitio, así que es cambiar dos líneas del mismo `docker-compose.yml` —la de Evolution y la de n8n— y reiniciar. **Antes de hacerlo hay que confirmar que las sesiones de WhatsApp viven en un volumen**, o reiniciar Evolution obligaría a re-escanear los QR de todos | Diego + Claude |
