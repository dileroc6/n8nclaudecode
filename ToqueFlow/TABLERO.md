...
| 141 | 🟡 **Pasar LuxeSmile y Zoe al estándar — Zoe hecha** | ✅ **Zoe está dentro de la estructura nueva, apagada** (10-sep, por decisión de Diego: *«no se va a necesitar, está congelado, pero dejémoslo en la estructura nueva apagado»*). Empresa, agente y **5 documentos de conocimiento** sacados del sistema viejo del repo —catálogo con precios, sedes, horarios y las dos frases de cumplimiento que son de la casa—. **El agente nace apagado y apuntando a `zoe-sin-conectar`, una instancia que no existe en Evolution**, así que no puede contestarle a nadie ni aunque alguien lo encienda por error. Comprobado: 0 productos encendidos y el vigilante la lee como «apagado», sin avisar. **Falta LuxeSmile**, que es el mismo trabajo | Diego |
...
| 144 | 🟡 **Evolution sigue devolviendo el token de cada instancia** | `AUTHENTICATION_EXPOSE_IN_FETCH_INSTANCES=false` **está aplicada dentro del contenedor** —lo comprobé— y aun así `fetchInstances` devuelve el `token` de cada instancia. O esa variable no hace lo que dice su nombre en la 2.3.7, o hay otra que la manda. **Ya no es grave:** ese token solo se alcanza con la llave nueva, que es aleatoria. Queda abierto porque comprobé que no está cerrado, y decir que algo está cerrado sin haberlo mirado es peor que dejarlo abierto | Claude |
