...
| 131 | 🔴 **El arreglo del `@lid` está escrito y no se puede subir** | El nodo y el envío ya están corregidos en `workflows/agente-atencion-generico.json`, y hay una prueba nueva —`pruebas/calidad/extraer-mensaje.cjs`— que corre **el código real del nodo sacado del JSON**, sin necesitar n8n ni Evolution ni la base: 8 casos, todos pasan. Cazan las tres cosas que importan: que un `@lid` nunca se tome como teléfono, que uno sin número real no cree un contacto basura, y que **nunca se conteste al número del propio negocio**. **Bloqueado por la `N8N_API_KEY` muerta**, que también impide ver el payload real para confirmar en qué campo hermano viaja el número | Diego |
