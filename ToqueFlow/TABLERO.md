...
| 118 | 🚨 **22 secretos escritos a mano dentro de los flujos de n8n** | Era «el receptor sigue con el código del piloto». Al ir a arreglarlo el 10-sep resultó **mucho más grande**: la comprobación nueva de `auditoria-n8n.cjs` encuentra **22 secretos en texto plano** repartidos en 15 flujos activos. Lo peor es **la llave de Evolution (`leadai…`), repetida en ~15 nodos**: quien la tenga puede mandar WhatsApp **como cualquier cliente**. También están la **firma del agente** dentro de `WA Router` —la llave que abre el agente— una **llave de Supabase** en `admin-evolution`, y credenciales `Basic` en el bot de Telegram de FerreteríaYa. Lo ve cualquiera que entre a n8n o exporte un flujo. **No se puede arreglar de una:** rotar la llave de Evolution tumba el WhatsApp de TODOS los clientes a la vez, así que va coordinado — primero las variables de entorno en el VPS (`TOQUE_AGENTE_FIRMA` y `EVOLUTION_API_KEY` **no existen** hoy en n8n; lo comprobé), después los flujos, y al final la rotación | Diego + Claude |
