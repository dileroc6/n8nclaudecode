...
| 107 | 🟠 **Subir al VPS el script de alertas con la firma** | Es lo único que quedó de la fila 102. El webhook `alerta-recursos` no comprueba quién llama, así que cualquiera puede mandarles alertas falsas del VPS a los dos socios —o peor, tapar las de verdad entre el ruido—. **El script ya está listo en el repo** (`ToqueFlow/infra/alerta-recursos.sh`, manda `X-Toque-Signature`). No cerré el webhook todavía porque cerrarlo antes de que subas el script apaga las alertas. Los pasos: subir el script a `/opt/toque/`, poner `TOQUE_ALERTA_FIRMA=<la firma>` en el crontab, avisarme, y yo cierro el webhook y lo verifico | Diego |
