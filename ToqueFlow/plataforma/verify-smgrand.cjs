// verify-smgrand.cjs — verifica empresa + usuario + flow de SM Grand Hotel.
...
(async () => {
  const co = (await get('companies?slug=eq.sm-grand-hotel&select=id,name,city,status,logo_url'))[0];
  ...
  const flows = await get('flows?company_id=eq.'+co.id+'&select=name,type,kind,status,tool_url,last_label&order=position.asc');
  console.log('FLOWS (' + flows.length + '):');
  for (const f of flows) console.log('  • "' + f.name + '" [' + f.type + ', ' + f.status + '] → ' + f.tool_url + ' | ' + f.last_label);
